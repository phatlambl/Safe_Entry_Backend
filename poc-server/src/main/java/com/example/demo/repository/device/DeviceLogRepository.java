...
public interface DeviceLogRepository extends JpaRepository<DeviceLog, Integer> {
	...
	Page<DeviceLog> findByUserIdAndTimestampGreaterThanEqualAndTimestampLessThanEqual(String userId, Long fromTimestamp,
			Long toTimestamp, Pageable pageable);
	...
	@Query(value = "select * from device_log l left join user u on l.user_id = u.id left join device d on l.device_id = d.id where l.name LIKE ?1 "
	...
	@Query(value = "select * from device_log l left join user u on l.user_id = u.id left join device d on l.device_id = d.id where l.name LIKE ?1 "
			+ " AND l.timestamp >= ?2 AND l.timestamp <= ?3", nativeQuery = true)
	public Page<DeviceLog> findByFilter(String name, Long fromTime, Long toTime, Pageable pageable);
